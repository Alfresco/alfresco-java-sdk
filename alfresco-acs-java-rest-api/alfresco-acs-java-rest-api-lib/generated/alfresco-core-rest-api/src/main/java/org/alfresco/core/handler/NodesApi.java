...
public interface NodesApi {
    ...
    @RequestMapping(value = "/nodes/{nodeId}/sources",
        ...
        method = RequestMethod.GET)
    @CollectionFormat(feign.CollectionFormat.CSV)
    ResponseEntity<NodeAssociationPaging> listSourceAssociations(@ApiParam(value = "The identifier of a target node.",required=true) @PathVariable("nodeId") String nodeId,@ApiParam(value = "Optionally filter the list by **assocType**. Here's an example:  *   ```where=(assocType='my:specialAssocType')``` ") @Valid @RequestParam(value = "where", required = false) String where,@ApiParam(value = "Returns additional information about the node. The following optional fields can be requested: * allowableOperations * aspectNames * isLink * isFavorite * isDirectLinkEnabled * isLocked * path * properties ") @Valid @RequestParam(value = "include", required = false) List<String> include,@ApiParam(value = "A list of field names.  You can use this parameter to restrict the fields returned within a response if, for example, you want to save on overall bandwidth.  The list applies to a returned individual entity or entries within a collection.  If the API method also supports the **include** parameter, then the fields specified in the **include** parameter are returned in addition to those specified in the **fields** parameter. ") @Valid @RequestParam(value = "fields", required = false) List<String> fields);
    ...
    @ApiOperation(value = "Generate a direct access content URL", nickname = "requestNodeDirectAccessUrl", notes = "**Note:** this endpoint is available in Alfresco 7.1 and newer versions. Generate a direct access content url for the given **nodeId**. ", response = DirectAccessUrlEntry.class, authorizations = {
        @Authorization(value = "basicAuth")
    }, tags={ "nodes", })
    @ApiResponses(value = { 
