...
/**
 * Unit tests for {@link NodeMovedFilter}.
 */
public class NodeMovedFilterTest {

    private final NodeMovedFilter nodeMovedFilter = NodeMovedFilter.get();

    @Test
    public void should_testTrue_when_nodeMovedEventIsSent() {
        final NodeResource nodeResource = NodeResource.builder()
                .build();
        final NodeResource nodeBeforeResource = NodeResource.builder()
                .setPrimaryHierarchy(List.of("level-1", "level-2"))
                .build();
